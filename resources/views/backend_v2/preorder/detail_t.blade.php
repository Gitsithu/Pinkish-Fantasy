...
<div class="row" style="width:100%!important;margin-top:100px!important;">
    ...
    <div class="row" id="row-margin-control" style="width:100%!important;margin-bottom:50px!important;">
        ...
        @if($order->status == 1)
        <div class="col-md-4">
            <form action="/admin/preorder/change/{{ $parameter }}" method="post" >
                <!-- <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>"> -->
      {{csrf_field()}}
      {{ method_field('PATCH') }}
                   <button type="submit"  class="form-control btn btn-success" style="font-weight:bold;padding-top:10px;height:40px;" id="btn_new">Make Order Placed</button>
                      </form>
        </div>
        <div class="col-md-4">
            <form action="/admin/preorder/confirm/{{ $parameter }}" method="post" >
                <!-- <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>"> -->
      {{csrf_field()}}
      {{ method_field('PATCH') }}
                   <button type="submit"  class="form-control btn btn-success" style="font-weight:bold;padding-top:10px;height:40px;background-color:#f5a8ae!important;border:#f5a8ae!important;" id="btn_new">Confirm</button>
                      </form>
           
        </div>
        <div class="col-md-4">
            <form action="/admin/preorder/cancel/{{ $parameter }}" method="post" >
                <!-- <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>"> -->
      {{csrf_field()}}
      {{ method_field('PATCH') }}
                   <button type="submit"  class="form-control btn btn-danger" style="font-weight:bold;padding-top:10px;height:40px;" id="btn_new">Cancel</button>
                      </form>
      ...
      {{csrf_field()}}
      {{ method_field('PATCH') }}
                   <button type="submit"  class="form-control btn btn-warning" style="font-weight:bold;padding-top:10px;height:40px;" id="btn_new">Make Order Placed</button>
                      </form>
        </div>
        <div class="col-md-4">
        </div>
